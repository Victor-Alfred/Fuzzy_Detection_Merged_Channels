...
for ww = 1:n_subfolders
    ...
    if exist([split_tifs_individual, '../../Yellow_binary'],'dir') == 0
        mkdir(split_tifs_individual,'../../Yellow_binary');
    end
    Yellow_binary = [split_tifs_individual,'../../Yellow_binary'];

     % create individual folders for thresholded images
    if exist ([filedir, ['/Yellow_binary/', num2str(ww)], 'dir']) == 0
        mkdir (filedir, ['/Yellow_binary/', num2str(ww)]);
    end
    Yellow_binary_files = [filedir, ['/Yellow_binary/', num2str(ww)]];
    ...
    for kk = 1:numel(files_split)
        ...
        img2 = uint8(255) - img; 
        img2 = rgb2gray(img2);
        img2 = imbinarize(img2); %figure, imshow(img2)
        img2 = medfilt2(img2); 
